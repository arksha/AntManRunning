...
//    this->addChild(blueSky);
    ...
    if ( map_index == -1 ) {
        _map = TMXTiledMap::create("tutorial_new_2/tutorial.tmx");
    }
    else {
        switch( map_index % 5 ) {
            case 0:
...
//                break;
    ...
    speedup = MenuItemImage::create( "GameAssets/gamebutton/sprintbefore.png", "GameAssets/gamebutton/sprintpress.png" ,CC_CALLBACK_1(GameLayer::speed_up_callback, this));
    speedup->setPosition( size.width * 0.9f, size.height * 0.15f );
    speedup->setScale( 0.5f );
    
    MenuItemImage* small = MenuItemImage::create( "GameAssets/gamebutton/sizebefore.png", "GameAssets/gamebutton/sizepress.png",CC_CALLBACK_1(GameLayer::Shrink_Magnify, this));
    small->setPosition( size.width * 0.1f, size.height * 0.15f );
    small->setScale( 0.5f );
    
    
    MenuItemImage* pause = MenuItemImage::create( "GameAssets/pause.png", "GameAssets/pause.png",CC_CALLBACK_1(GameLayer::Pause, this));
...
void GameLayer::speed_up_callback( Ref* ref ) {
    ...
    if ( !pauseflag ) {
        ...
        if ( item->energy >= 5 ) {
            ...
            auto image = Sprite::create("GameAssets/gamebutton/sprintbefore.png");
            speedup->setNormalImage(image);
        ...
        Rect playerRect = player->boundingBox();
            ...
            if (tileRect.intersectsRect(playerRect)){ //金币碰撞处理 可以用记分
                  ...
                  if (gid ) {
                      ...
                      if ( item->energy >= 5 ) {
                          auto image = Sprite::create("GameAssets/gamebutton/sprintavailable.png");
                          speedup->setNormalImage(image);
                      }
...
void GameLayer::Pause(Ref* ref ) {
    ...
    if ( pauseflag == false ) {
        pauseflag=true;
        if(_pauseMenu == NULL){
            _pauseMenu = Sprite::create( "GameAssets/bgpause.png" );
            _pauseMenu->setPosition(Point(visibleSize.width/2 + origin.x, visibleSize.height/2 + origin.y) );
            addChild(_pauseMenu);
        }
